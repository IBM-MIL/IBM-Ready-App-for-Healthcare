...
class LoginViewController: UIViewController, UITextFieldDelegate, CustomAlertViewExerciseDelegate {
    ...
    override func viewDidLoad() {
        ...
        var alertText = NSLocalizedString("Login Failed.", comment: "")
        var buttonText = NSLocalizedString("Try another ID or Password", comment: "")
    ...
    */
    ...
    :param: result the result of authentication
    */
        ...
        if (result == LogInResultType.Success) {
            ...
            if (appDelegate.username == "user2" && appDelegate.password == "password2") {
                ...
                if !didAlreadyPopulateHealthKit {
                    ...
                    var alert = UIAlertController(title: NSLocalizedString("HealthKit Already Populated", comment: "n/a"), message: NSLocalizedString("You have already populated your HealthKit with data.  To remove this data from your phone, either Log Out or delete the app from your phone.", comment: "n/a"), preferredStyle: .Alert)
                    let OKAction = UIAlertAction(title: NSLocalizedString("OK", comment: "n/a"), style: .Default) { (action) in
                        HealthKitManager.healthKitManager.getPermissionsForHealthKit(self.progressToDashboard)
                    }
    ...
    */
    func timeOutDone() {
        if !haveLoginResult {
            ...
            var alert = UIAlertController(title: NSLocalizedString("Cannot communicate with the server", comment: "n/a"), message: NSLocalizedString("Please try again later", comment: "n/a"), preferredStyle: .Alert)
    ...
    :param: shouldProgress Bool that determines if HealthKit access was granted.
    */
    func progressToDashboard(shouldProgress: Bool) {
        if (shouldProgress) {
            ...
            var alert = UIAlertController(title: NSLocalizedString("Please Allow HealthKit Access", comment: "n/a"), message: NSLocalizedString("You must allow access to HealthKit to properly use Physio", comment: "n/a"), preferredStyle: .Alert)
            alert.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "n/a"), style: .Default, handler: nil))
            self.presentViewController(alert, animated: true, completion: nil)
        }
    }
    ...
    :param: textField The textfield that is selected at the time the return key is pressed
    */
    ...
    :param: notification notification that keyboard has become active
    */
    ...
    :param: notification notificaiton that the keyboard is dismissing
    */
