...
/**
 * Responsible for querying the Cloudant database directly via the Cloudant Java
 * API. The public facing methods defined here are typically invoked by a
 * MobileFirst Platform (MFP) adapter.
 * 
 * Note: This class is not designed for extension and thus has been marked as
 * final.
 * 
 * @author jpetitt
 */
public final class CloudantDBConnector {
	private static final Logger LOGGER = Logger
			.getLogger(CloudantDBConnector.class.getSimpleName());
	...
	/**
	 * Returns a single instance of the CloudantDBConnector class.
	 * Synchronization is in place in order to ensure only one instance is ever
	 * created. Due to the class's constructor being overridden and marked as
	 * private, this is the only way to instantiate the class.
	 * 
	...
	public User getUser(String username) {
		...
		List<User> users = service.getDatabase()
				.view("physio_design_doc/users").key(username)
				.includeDocs(true).query(User.class);
	...
	/**
	 * Collects all routines for a given user in the desired locale. If no
	 * routines are found, an empty list is returned.
	 * 
	 * @param user
	 * @param locale
	...
	public List<Routine> getRoutines(User user, String locale) {
		...
		for (String category : routineCategories) {
			List<Routine> routines = service.getDatabase()
					.view("physio_design_doc/routines").key(category, locale)
					.includeDocs(true).query(Routine.class);
	...
	/**
	 * Returns the list of exercises associated with the given routine id and
	 * locale. If no routine is found for the specified id, then null is
	 * returned. Otherwise, a list of zero or more exercises is returned.
	 * 
	 * @param user
	 * @param locale
	 * @param routineId
	 * @return A list of zero or more exercises. If a routine does not exist for
	 *         the specified id, then null is returned.
	 */
	public List<Exercise> getExercises(User user, String locale,
			String routineId) {
	...
	/**
	 * Returns the Questionnaire for the associated User and locale. If no
	 * questionnaire exists, null is returned.
	 * 
	 * @param user
	 * @param locale
	 * @return The questionnaire for the associated User and locale. Null is
	 *         returned if no questionnaire exists.
	 */
	public Questionnaire getQuestionnaire(User user, String locale) {
		LOGGER.log(Level.INFO, "user: " + user);

		String questionnaireCategory = user.getQuestionnaire();
		LOGGER.log(Level.INFO, "questionnaire category: "
				+ questionnaireCategory);
		List<Questionnaire> questionnaires = service.getDatabase()
				.view("physio_design_doc/questionnaires")
				.key(questionnaireCategory, locale).includeDocs(true)
	...
	public static void main(String[] args) {
		...
		List<Exercise> exercises = connector
				.getExercises(user, "en", routineId);
